...
La fase de seguridad añade SSO WordPress RS256, sesión interna HS256 y cierre por 20 minutos de inactividad. Desarrollo sigue con `APP_ENV=development` y `AUTH_ENABLED=false`; producción exige autenticación, secreto independiente y clave pública RSA. Producción usa **Apache HTTPS → Docker Compose → Gunicorn → Flask**, sin despliegue realizado desde este workspace. Detalle técnico: [docs/AUTENTICACION_SSO.md](docs/AUTENTICACION_SSO.md).

CI/CD: [.github/workflows/ci-cd-gcp.yml](.github/workflows/ci-cd-gcp.yml) prueba cada push/PR y despliega automáticamente una imagen Docker por SSH tras un push a la rama de producción cuando se habilita. Instrucciones separadas: [GitHub y Secrets](docs/GITHUB_ACTIONS.md), [GCP y Docker](docs/GCP_DOCKER.md), [WordPress y SSO](docs/WORDPRESS_SSO.md). Con Woody existente no necesita otro plugin.
...
**Una única instancia/servidor con filesystem común.** Contenedores con discos separados no comparten el lock. Apps Script original tampoco participa en ese bloqueo: no opere simultáneamente los dos sistemas para escribir inventarios. El despliegue usa [Dockerfile](Dockerfile) y [compose.yaml](compose.yaml), Gunicorn con un worker, cuatro hilos y bind `127.0.0.1:8000`, mediante red del host en Linux. Apache termina HTTPS en la VM. Credenciales, token y configuración se montan externamente. Las actualizaciones incluyen una pausa de 320 segundos por la caché SSO en memoria; consulte [GCP](docs/GCP_DOCKER.md). La imagen se construye y verifica en CI; no se ha desplegado desde este workspace.
