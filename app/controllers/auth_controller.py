...
def register_auth(app):
    ...
    if app.config["AUTH_ENABLED"]:
        ...
        app.extensions["session_auth"] = SessionAuthService(
            app.config,
            public_key,
        )
    else:
        app.extensions["session_auth"] = None
    ...
    def protect():
        ...
        try:
            g.auth_session = service().read_session(
                request.cookies.get(
                    app.config["SESSION_JWT_COOKIE_NAME"]
                )
            )
        ...
        if request.method not in {
            ...
            if (
                ...
                return jsonify(
                    correcto=False,
                    mensaje="Origen de solicitud no permitido.",
                ), 403
    ...
    def session_headers(response):
        ...
        if (
            ...
            response.headers["X-Session-Expires-At"] = str(
                claims["exp"]
            )

            response.headers["X-Session-Activity-At"] = str(
                claims["act"]
            )

            response.headers["X-Server-Time"] = str(
                service().now()
            )

            response.headers["X-Session-Context"] = (
                service().context(claims)
            )
...
def sso():
    if not current_app.config["AUTH_ENABLED"]:
        return jsonify(
            correcto=False,
            mensaje="SSO no está habilitado.",
        ), 404

    if (
        request.query_string
        or request.content_length is None
        or request.content_length > 16384
    ):
        return access_response()
    ...
    try:
        ...
        g.auth_session = service().consume_sso(
            tokens[0]
        )

    except jwt.InvalidTokenError:
        current_app.logger.info(
            "SSO rechazado: token inválido, expirado o replay."
        )
        return access_response()
...
def activity():
    ...
    if (
        ...
        return jsonify(
            correcto=False,
            mensaje="Actividad inválida.",
        ), 400

    try:
        g.auth_session = service().renew(
            g.auth_session,
            math.ceil(idle),
        )
