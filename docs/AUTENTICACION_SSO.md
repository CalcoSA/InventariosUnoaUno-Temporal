...
## Operación actual con Docker

El despliegue usa una sola instancia Gunicorn dentro de Docker Compose con red del host en Linux y Apache local para HTTPS. Se conserva la confianza únicamente en proxies de loopback. Pasos actuales: [GCP](GCP_DOCKER.md), [GitHub](GITHUB_ACTIONS.md) y [WordPress](WORDPRESS_SSO.md). El archivo PHP puede instalarse con Woody o como plugin independiente, una sola vez. El ejecutor aplica una pausa de 320 segundos al reemplazar el proceso por la caché de uso único en memoria; no garantiza persistencia de esa caché ante caídas inesperadas.
