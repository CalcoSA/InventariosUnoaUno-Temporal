...
Estados: **OK** = código y comportamiento local verificados; **BLOCKED_BY_CREDENTIALS** = implementación completa y pruebas con mocks, pero falta validar la operación real con OAuth; **COMPATIBILITY_ADAPTER** = requiere el adaptador FormApp incluido, todavía sin desplegar. No hay operaciones de producción ejecutadas durante la migración.

| Función original | Implementación Python | Estado | Observaciones |
|---|---|---|---|
| `iniciarCreacionMasiva` | `PdvCreationService.start_mass_creation` | BLOCKED_BY_CREDENTIALS | Carpeta padre, BC01, K1:L3, lock exclusivo y ciclo secuencial. También requiere el adaptador. |
| `procesarSiguientePDV` | `PdvCreationService.process_next_pdv` | BLOCKED_BY_CREDENTIALS | Primer XLSX no registrado, 3 segundos tras conversión, estados y continuación tras ERROR; pausa de 60 segundos. |
...
| `registrarBC01_` | `PdvCreationService.register_bc01` | BLOCKED_BY_CREDENTIALS | Nombre exacto del maestro sin extensión, no duplica ID; toma Configuración y cantidad de filas. |
...
| `obtenerLibroMaestro_` | `MasterSheetRepository` | BLOCKED_BY_CREDENTIALS | Mismo ID en .env. |
| `obtenerPuntosVenta` | `InventoryService.get_points_of_sale` | BLOCKED_BY_CREDENTIALS | B/C/D, LISTO, trim, deduplicación por nombre exacto y orden español. |
| `obtenerCategorias` | `InventoryService.get_categories` | BLOCKED_BY_CREDENTIALS | Categorías únicas extraídas de productos. |
| `obtenerProductos` | `InventoryService.get_products` | BLOCKED_BY_CREDENTIALS | Encabezados alternativos, fallback A:D, display/raw separados, categoría por defecto Uno a Uno. |
| `guardarInventario` | `InventoryService.save_inventory` | BLOCKED_BY_CREDENTIALS | UUID común, 12 columnas, cantidades y factor en backend, bloqueo y duplicado; mocks cubren envío completo. |
| `actualizarResumenInventario_` | `InventorySummaryService.update_pdv_summary` | BLOCKED_BY_CREDENTIALS | Fecha/PDV normalizado/item, primer factor, suma Cerrado/Abierto y recalcula físico; limpia solo contenidos. |
| `registrarEnResumenGeneral_` | `InventorySummaryService.register_general_summary` | BLOCKED_BY_CREDENTIALS | Incrementa Cerrado, Abierto y físico anterior; reemplaza metadatos con el registro nuevo. |
| `reconstruirResumenGeneral` | `InventorySummaryService.rebuild_general_summary` | BLOCKED_BY_CREDENTIALS | Solo LISTO, recalcula físico individual, agrupa, ordena, limpia y reescribe. CLI pide ejecución consciente. |
...
| `obtenerHojaResumenGeneral_` | `GeneralSummaryRepository.ensure_sheet` | BLOCKED_BY_CREDENTIALS | Reutiliza hoja exacta o renombra la primera vacía, o crea otra. |
| `prepararHojaResumenGeneral_` | `GeneralSummaryRepository.prepare` | BLOCKED_BY_CREDENTIALS | Encabezado solo si la hoja está vacía. |
| `obtenerFactoresPorItem_` | `PdvSheetRepository.factors` | BLOCKED_BY_CREDENTIALS | Exige columna Item y Factor, usa valores raw y última ocurrencia del item. |
...
| `aplicarFormatoResumen_` | `GoogleSheetsService.format_header` | BLOCKED_BY_CREDENTIALS | #4A2B14, blanco, negrita, congelación y autoajuste. |
| `claveFecha_` | `utils.date_key` | OK | Date en huso configurado, ISO y DD/MM o DD-MM; no interpreta otros formatos. |
| `obtenerLibroPDV_` | `MasterSheetRepository.pdv_book` / `PdvSheetRepository.book` | BLOCKED_BY_CREDENTIALS | Compara el nombre exacto del PDV; estado LISTO y URL existente. |
| `buscarHoja_` | `GoogleSheetsService.find_sheet(normalized=True)` | BLOCKED_BY_CREDENTIALS | Usa normalización backend; la creación masiva conserva la variante simple del otro script. |
...
| `actualizarTodasLasBasesUDM` | `UdmService.update_all` / `update_pdv` | BLOCKED_BY_CREDENTIALS | Por nombre normalizado, crea columnas faltantes, conserva valores no encontrados, log de 6 columnas. |
...
| `cargarPuntosVenta` | JS + GET `/api/puntos-venta` | BLOCKED_BY_CREDENTIALS | Callback original conservado; transporte fetch. |
| `cargarCategoriasPDV` | JS + GET `/api/categorias` | BLOCKED_BY_CREDENTIALS | Mismos selectores, estados y mensajes. |
| `comenzarInventario` | JS + GET `/api/productos` | BLOCKED_BY_CREDENTIALS | Mismas validaciones, carga, borrador y pantalla. |
...
| `guardarInventario` (frontend) | JS + POST `/api/inventarios` | BLOCKED_BY_CREDENTIALS | Payload y mensajes conservados; error no elimina borrador. |
...
- La ordenación usa Unicode Collation con tratamiento de ñ y orden numérico de items. Se comprueba con el motor JavaScript en las pruebas; la aceptación con los nombres/códigos reales requiere las credenciales.
